...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
    	SortedMap<T3Action,T3State> finalTransitions = new TreeMap<T3Action,T3State>();
    	for (int row = 0 ; row < this.state.length; row++) {
            for (int col = 0 ; col < this.state[row].length ; col++) {
            	int[] numOfMoves = this.getMoves();
            	for (int i = 0; i < numOfMoves.length; i++) {
            		T3Action newAct = new T3Action(row, col, numOfMoves[i]);
                	if (this.isValidAction(newAct)) {
                		finalTransitions.put(newAct, this.getNextState(newAct));
                	}
            	}	
            }
        }
